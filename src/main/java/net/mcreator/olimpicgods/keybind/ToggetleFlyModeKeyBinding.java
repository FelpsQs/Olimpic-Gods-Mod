...
package net.mcreator.olimpicgods.keybind;
...
import net.mcreator.olimpicgods.procedures.StopflyProcedure;
...
public class ToggetleFlyModeKeyBinding extends OlimpicGodsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	private long lastpress = 0;
	public ToggetleFlyModeKeyBinding(OlimpicGodsModElements instance) {
		super(instance, 4);
	...
	public void initElements() {
		keys = new KeyBinding("key.olimpic_gods.toggetle_fly_mode", GLFW.GLFW_KEY_F8, "key.categories.movement");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					OlimpicGodsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
		...
		if (type == 1) {
			{
				...
				StopflyProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
